...
Dynamically typed languages are the foundation on which the modern website is
built. Javascript is, primarily, the language that powers interactive websites
where elements on the page can be added and removed and changed in real time
based on what the user does. PHP is the most commonly used language for server-side scripting. This is what allows web pages to have dynamic
content. That a page can be personalised on a per-user basis or generated as
it is needed to exist is, for the majority of web sites, brought about by these
languages. Several other dynamically typed languages are commonly used for
server-side scripting. These include ruby, python and javascript.

\subsection{Role of Interpreters in Dynamically Typed Languages}
Dynamically typed languages are commonly paired with an interpreter. That is,
these languages are not compiled to machine code that can be run on some architecture.
Instead, it is the job of a seperate program to determine the meaning of the program
and perform the instructions on actual hardware. Sometimes this is done by parsing
the text of the program directly and executing the code line by line, but a more common (is it??) approach is the compile the source code to an intermediary langauge
called byte-code. The interpreter reads and executes the byte code instead of the 
original program. This is easier and faster becuse the byte-code has a very simple,
consistent structure.

\subsection{Virtual Machines}

To best describe the virtual machine presented in this treatise,
...
Virtual machines come in two varieties: \emph{process }virtual machines
...
A dynamically typed language is one in which the type information
...
The project is to develop an implementation of a VM of this nature
...
The risks of the project are mainly about its structure and how it
...
In \emph{fig1} we see that the first instruction as currently performing
...
Predicting a branch means we must predict if that branch is taken
...
Modern processors make use of different levels of cache to allow
...
The simplest way to implement a virtual machine is to make use of
...
A faster alternative to switch dispatch is direct threading\cite{Ertl}.
...
Another method is indirect threading. In indirect threading we keep
...
For a register machine, a similar piece of code would have values
...
Both the JVM and CLR make use of JIT compilation {[}x{]}. JIT compilation allows for code that is executed often enough to be compiled into
...
Yuhne Shi\cite{Shi2007} found that a well-implemented register VM
is a more efficient option when speed of execution is more important
than the size of the code to be executed.

Ertl and Gregg found that in their benchmarks 3.2\%\textendash 13\%
...
In our VM instructions for programs are stored as opcodes only with
