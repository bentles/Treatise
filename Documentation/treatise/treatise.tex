...
\begin{titlepage}
	\begin{center}
		...
		{\color{NMMUBlue}{\huge \bfseries A JIT-Less Type-Mapped
          ...
          Supervisor: \emph{Dr. Kevin Naud\'{e}}}
...
\begin{figure}[!htb]
	...
	\node (v4) at (3,-3) {$add\_(n-1)\_(n-2)$};
	\node (v5) at (3,-4) {$add\_(n-1)\_(n-1)$};
	\draw 
    (v0.east) -- (v1.west)
	(v0.east) -- (v2.west)
	(v0.east) -- (v4.west)
...
\begin{figure}[!htb]
	...
	\node (v2) at (3,-1) {$add\_0\_1$};
	\node[draw=white] (v3) at (3.75,-2) {$\vdots$};
	\node (v4) at (3,-3) {$add\_5\_4$};
	\node (v5) at (3,-4) {$add\_5\_5$};
	\draw 
    (v0.east) -- (v1.west)
	(v0.east) -- (v2.west)
	(v0.east) -- (v4.west)
...
\begin{figure}[!htb]
	\centering
	\begin{tikzpicture}
	\tikzstyle{every node}=[draw,shape=rectangle,anchor=west,minimum width=60pt,minimum height=18pt];
	\node (v0) at (0,-2) {$add$};

	\node (v1) at (3,0)  {$add\_0\_0$ || $error$};
	\node (v2) at (3,-1) {$add\_0\_1$ || $error$};
	\node[draw=white] (v3) at (4,-2) {$\vdots$};
	\node (v4) at (3,-3) {$add\_5\_4$ || $error$};
	\node (v5) at (3,-4) {$add\_5\_5$ || $error$};
...
Figure \ref{fig:op-case-expansion-implementation-2} shows how operand
case expansion works for the OTVM. There are two internal
implementations available for each possible operand combination for
the logical \verb|add| instruction. These two internal instructions
are used to implement the functionality of the if and else blocks
present in the OVM implementation.

The \verb|TS| register is used to choose between these
implementations. Figure \ref{fig:op-case-with-TS} shows the chosen
implementation for the operand cases for two different \verb|TS|
values. A \verb|TS| value of $100000_2 << 11$ means that $g0$ holds a
pointer. Since \verb|add| acts only on integers any operand case
containing a zero (like \verb|add_0_0| or \verb|add_2_0|) is an error
case. This is seen in figure \ref{ts-100000} where the
type-appropriate implementations have been selected.
...
\begin{figure}[!htb]
  \begin{subfigure}[t]{0.5\linewidth} 
	...
	\begin{tikzpicture}[scale=1]
      ...
      \node (v1) at (3,0)  {$error$};
      \node (v2) at (3,-1) {$error$};
      \node[draw=white] (v3) at (3,-2) {$\vdots$};
      \node (v4) at (3,-3) {$add\_5\_4$};
      \node (v5) at (3,-4) {$add\_5\_5$};
  ...
  \begin{subfigure}[t]{0.5\linewidth}
	...
	\begin{tikzpicture}[scale=1]
      ...
      (v0.east) -- (v5.west);
	\end{tikzpicture}
    \caption{TS = $000001_2 << 11$}
  \end{subfigure}
  \caption{Operand Case Expansion With Type Mapping}
  \label{fig:op-case-with-TS}	
\end{figure}
