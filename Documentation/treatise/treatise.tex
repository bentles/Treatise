...
\section{Labels As Addresses and Dispatch}
...
For the Operand-Type and Operand Disptach VMs there are, in some
cases, multiple internal instructions for each logical
instruction. For these instructions there is a many-to-one
correspondence between the internal instructions and the logical
instructions they implement.
...
For the Conventional VM, the \verb|add| instruction has only a single
internal instruction that makes up its implementation:
\begin{lstlisting}
add:
{
...
For the Operand Dispatch VM, the \verb|add| instruction is implemented
using 36 internal instructions of the following form:
...
For the Operand-Type Dispatch VM, the \verb|add| instruction is
implemented using 37 internal instructions. The last instruction
