...
A further two registers are found in all VMs. that is the instruction pointer, which will be referred to as \verb|IP| and the frame pointer which will be referred to as \verb|FP|. These are a 16-bit-integer pointer and a value pointer respectively.
...
\begin{description}
	\item[register number] These operands are numbers from 0 to 5 that address the six registers. For the conventional VM these are an explicit part of the instruction word as shown in Figure[x]. For the other two VMs these operands are implicitly part of the instruction word.
