...
this manner is known as \emph{just-in-time compilation} (JIT
compilation).
...
pipeline{[}x{]} which divides instruction processing into the
...
In figure~\ref{fig:pipeline} we see that the first instruction as currently
...
prediction misses \cite{EfficientInterpreters}. They also found that direct
...
time. In \emph{fig 2} we see a 16-bit instruction composed of a 4-bit
